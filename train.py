...
for epoch in iter_counter.training_epochs():
    ...
    for i, data_i in enumerate(tqdm(dataloader), start=iter_counter.epoch_iter):
        ...
        if iter_counter.needs_displaying():
            ...
            b, c, h, w = data_i['img_tensor'].size()

            visuals = OrderedDict([('train_1_original', data_i['img_tensor'].view(b*c, 1, h, w)),
                                   ('train_2_fake', fake_image),
                                   ('train_3_diff', torch.abs(data_i['img_tensor'].view(b*c, 1, h, w) - fake_image.detach().cpu()))
                                   ])
    ...
    for i, data_i in tqdm(enumerate(dataloader_val), desc='Validation images generating') :
        ...
        valid_losses = {}
        valid_losses['valid_L1'] = trainer.model.module.L1loss(fake_image, data_i['img_tensor'].cuda().view(b*c, 1, h, w)) * opt.lambda_rec
        visualizer.print_current_errors(epoch, iter_counter.epoch_iter,
                                        valid_losses, iter_counter.time_per_iter)
        visualizer.plot_current_errors(valid_losses, iter_counter.total_steps_so_far)
        diff = torch.abs(data_i['img_tensor'].view(b*c, 1, h, w) - fake_image.detach().cpu())
        visuals = OrderedDict([('valid_1_original', data_i['img_tensor'].view(b*c, 1, h, w)),
