...
class AnoGANModel(nn.Module) :
    ...
    def forward(self, data, mode):
        el_tensor, label = self.preprocess_input(data)
        if mode == 'generator':
            g_loss, fake_t = self.compute_generator_loss(el_tensor)
            return g_loss, fake_t
        elif mode == 'discriminator':
            d_loss = self.compute_discriminator_loss(el_tensor)
            return d_loss
        elif mode == 'inference' :
            self.netG.eval()
            self.netE.eval()
            with torch.no_grad():
                mu, _, _ = self.encoding(el_tensor, loss=False)
                zero = torch.zeros_like(mu).cuda()
                texture_information = [mu, zero]
    ...
    def preprocess_input(self, data):
        ...
        b, c, h, w = data['img_tensor'].size()
        data['img_tensor'] = data['img_tensor'].view(-1, 1, h, w)
